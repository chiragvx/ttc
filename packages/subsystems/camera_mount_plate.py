"""Camera Mount Plate -- Small plate with a 1/4 inch-20 clearance boss (thread not modeled)
...
_FRAGMENT = """\
## Subsystem: Camera Mount Plate
Small plate with a 1/4 inch-20 clearance boss (thread not modeled) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
CAMERA_MOUNT_PLATE = register_subsystem(Subsystem(
    name="camera_mount_plate",
    description="Small plate with a 1/4 inch-20 clearance boss -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=25.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("depth_mm", value=25.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("thickness_mm", value=4.0, min=1.0, max=15.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=6.4, min=2.0, max=12.0, unit='mm'),
