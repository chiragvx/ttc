"""Hinge With Pin -- Hinge + a dowel pin sized to the knuckle bore (represents the pin only; leaves modeled via the hinge subsystem)
...
_FRAGMENT = """\
## Subsystem: Hinge With Pin
Hinge + a dowel pin sized to the knuckle bore (represents the pin only; leaves modeled via the hinge subsystem) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
HINGE_WITH_PIN = register_subsystem(Subsystem(
    name="hinge_with_pin",
    description="Hinge + a dowel pin sized to the knuckle bore -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=4.0, min=2.0, max=12.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=0.5, min=0.1, max=3.0, unit='mm'),
        ParamSpec("height_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
