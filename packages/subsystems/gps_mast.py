"""Gps Mast -- Small mast standing the GPS module off the airframe/ground plane

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
9) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Gps Mast
Small mast standing the GPS module off the airframe/ground plane -- a solid rectangular bar (FDM/FFF or CNC).
...
GPS_MAST = register_subsystem(Subsystem(
    name="gps_mast",
    description="Small mast standing the GPS module off the airframe/ground plane -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=60.0, min=15.0, max=250.0, unit='mm'),
        ParamSpec("width_mm", value=8.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=8.0, min=3.0, max=40.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # box-bar centered at origin along local X — same shape family as flat_bar/longeron
))
