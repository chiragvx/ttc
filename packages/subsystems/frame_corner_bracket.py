"""Frame Corner Bracket -- 3-axis corner bracket for T-slot framing (modeled as a 2-flange corner bracket; the third axis not separately modeled)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Frame Corner Bracket
3-axis corner bracket for T-slot framing (modeled as a 2-flange corner bracket; the third axis not separately modeled) -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
FRAME_CORNER_BRACKET = register_subsystem(Subsystem(
    name="frame_corner_bracket",
    description="3-axis corner bracket for T-slot framing -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=25.0, min=10.0, max=60.0, unit='mm'),
        ParamSpec("leg_b_mm", value=25.0, min=10.0, max=60.0, unit='mm'),
        ParamSpec("width_mm", value=25.0, min=10.0, max=60.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.5, min=0.8, max=12.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
