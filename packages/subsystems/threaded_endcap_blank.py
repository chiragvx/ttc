"""Threaded Endcap Blank -- Endcap with a threaded-insert receiver in the center (thread not modeled)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
3) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Threaded Endcap Blank
Endcap with a threaded-insert receiver in the center (thread not modeled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
THREADED_ENDCAP_BLANK = register_subsystem(Subsystem(
    name="threaded_endcap_blank",
    description="Endcap with a threaded-insert receiver in the center -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=8.0, min=2.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=12.0, min=3.0, max=35.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
