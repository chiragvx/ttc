"""Knurled Nut -- Small cylindrical thumb nut with knurled OD (no knurl grooves)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
1) -- a plain solid cylinder. Fine surface/profile detail this part's real-world name
...
Small cylindrical thumb nut with knurled OD (no knurl grooves) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
KNURLED_NUT = register_subsystem(Subsystem(
    name="knurled_nut",
    description="Small cylindrical thumb nut with knurled OD -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=10.0, min=4.0, max=30.0, unit='mm'),
        ParamSpec("height_mm", value=6.0, min=2.0, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
