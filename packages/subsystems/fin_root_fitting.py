"""Fin Root Fitting -- Attachment fitting where the vertical fin meets the boom/fuselage

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
3) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Fin Root Fitting
Attachment fitting where the vertical fin meets the boom/fuselage -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
FIN_ROOT_FITTING = register_subsystem(Subsystem(
    name="fin_root_fitting",
    description="Attachment fitting where the vertical fin meets the boom/fuselage -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=40.0, min=15.0, max=120.0, unit='mm'),
        ParamSpec("leg_b_mm", value=40.0, min=15.0, max=120.0, unit='mm'),
