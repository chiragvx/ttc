"""T Handle -- T-shape handle grip (T cross-bar not modeled, approximated as a straight grip)
...
_FRAGMENT = """\
## Subsystem: T Handle
T-shape handle grip (T cross-bar not modeled, approximated as a straight grip) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
T_HANDLE = register_subsystem(Subsystem(
    name="T_handle",
    description="T-shape handle grip -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=16.0, min=6.0, max=45.0, unit='mm'),
        ParamSpec("height_mm", value=60.0, min=15.0, max=180.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
