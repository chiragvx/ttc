"""Spar Joiner Sleeve -- Sleeve that joins two spar sections (multi-piece wings)
...
_FRAGMENT = """\
## Subsystem: Spar Joiner Sleeve
Sleeve that joins two spar sections (multi-piece wings) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
SPAR_JOINER_SLEEVE = register_subsystem(Subsystem(
    name="spar_joiner_sleeve",
    description="Sleeve that joins two spar sections (multi-piece wings) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=18.0, min=6.0, max=60.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=12.0, min=3.0, max=50.0, unit='mm'),
        ParamSpec("height_mm", value=60.0, min=10.0, max=200.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
