"""Thrust Washer -- Annular thrust bearing washer (topologically = washer)
...
_FRAGMENT = """\
## Subsystem: Thrust Washer
Annular thrust bearing washer (topologically = washer) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
THRUST_WASHER = register_subsystem(Subsystem(
    name="thrust_washer",
    description="Annular thrust bearing washer (topologically = washer) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=20.0, min=6.0, max=70.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=10.0, min=2.0, max=60.0, unit='mm'),
        ParamSpec("height_mm", value=1.5, min=0.5, max=6.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
