"""Gear Door Hinge -- Hinge leaf for a retractable-gear bay door (geometry only, no deployment dynamics)
...
_FRAGMENT = """\
## Subsystem: Gear Door Hinge
Hinge leaf for a retractable-gear bay door (geometry only, no deployment dynamics) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
GEAR_DOOR_HINGE = register_subsystem(Subsystem(
    name="gear_door_hinge",
    description="Hinge leaf for a retractable-gear bay door (geometry only, no deployment dynamics) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    ...
    params=[
        ...
        ParamSpec("hole_dia_mm", value=3.5, min=1.5, max=8.0, unit='mm'),
