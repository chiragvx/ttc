"""Shock Strut Housing -- Housing for a shock strut (spring/damper not modelled)
...
_FRAGMENT = """\
## Subsystem: Shock Strut Housing
Housing for a shock strut (spring/damper not modelled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
SHOCK_STRUT_HOUSING = register_subsystem(Subsystem(
    name="shock_strut_housing",
    description="Housing for a shock strut (spring/damper not modelled) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=16.0, min=6.0, max=50.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=8.0, min=2.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=70.0, min=15.0, max=250.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
