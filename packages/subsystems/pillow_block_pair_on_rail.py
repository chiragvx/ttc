...
def _flange_face(p):
    """The flange's OUTER (bottom) face, z=0 -- where this block bolts flat against the rail (the
    `n_bolt_holes` pattern passes through the flange right at this face). Confirmed against
    `_build`: `flange = bd.Pos(0, 0, flange_thickness_mm/2) * bd.Cylinder(...)`, i.e. the flange
    spans z in [0, flange_thickness_mm] -- NOT centered at the origin the way
...
PILLOW_BLOCK_PAIR_ON_RAIL = register_subsystem(Subsystem(
    ...
    # (bolts down onto the rail); `shaft_tip` is the boss's free end. See `_flange_face`/`_shaft_tip`
