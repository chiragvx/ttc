"""Four Leg Stand -- Rectangular table with adjustable leg heights (represents the tabletop; legs are separate instances (see table.py for a fused variant))
...
_FRAGMENT = """\
## Subsystem: Four Leg Stand
Rectangular table with adjustable leg heights (represents the tabletop; legs are separate instances (see table.py for a fused variant)) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
FOUR_LEG_STAND = register_subsystem(Subsystem(
    name="four_leg_stand",
    description="Rectangular table with adjustable leg heights -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=100.0, min=25.0, max=350.0, unit='mm'),
        ParamSpec("depth_mm", value=70.0, min=18.0, max=280.0, unit='mm'),
        ParamSpec("thickness_mm", value=6.0, min=2.0, max=25.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=4.0, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=4, min=4, max=4, unit='count'),
        ParamSpec("margin_mm", value=10.0, min=3.0, max=30.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
