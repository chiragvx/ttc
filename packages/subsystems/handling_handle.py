"""Handling Handle -- Fixed carry handle on the airframe
...
_FRAGMENT = """\
## Subsystem: Handling Handle
Fixed carry handle on the airframe -- an extruded U-channel, open at the top and both ends (FDM/FFF or extruded stock).
...
HANDLING_HANDLE = register_subsystem(Subsystem(
    name="handling_handle",
    description="Fixed carry handle on the airframe -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=80.0, min=20.0, max=300.0, unit='mm'),
        ParamSpec("width_mm", value=15.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("wall_mm", value=3.0, min=1.0, max=12.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
