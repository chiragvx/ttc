"""Lcd 16X2 Bezel -- Bezel preset for a 16x2 LCD module

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
3) -- a flat plate with a central rectangular window/cutout plus four corner mounting
...
_FRAGMENT = """\
## Subsystem: Lcd 16X2 Bezel
Bezel preset for a 16x2 LCD module -- a flat plate with a central cutout and four corner mounting holes (FDM/FFF or CNC).
...
LCD_16X2_BEZEL = register_subsystem(Subsystem(
    name="LCD_16x2_bezel",
    description="Bezel preset for a 16x2 LCD module -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=80.0, min=20.0, max=200.0, unit='mm'),
        ParamSpec("height_mm", value=36.0, min=10.0, max=100.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
        ParamSpec("window_w_mm", value=66.0, min=10.0, max=180.0, unit='mm'),
        ParamSpec("window_h_mm", value=16.0, min=5.0, max=80.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=6.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=4.0, min=2.0, max=12.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
