"""C Channel -- Extruded C cross-section (open one side)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- an extruded U-channel (base + two side walls, open top and both ends), the SAME
...
_FRAGMENT = """\
## Subsystem: C Channel
Extruded C cross-section (open one side) -- an extruded U-channel, open at the top and both ends (FDM/FFF or extruded stock).
...
C_CHANNEL = register_subsystem(Subsystem(
    name="c_channel",
    description="Extruded C cross-section (open one side) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=300.0, min=30.0, max=1200.0, unit='mm'),
        ParamSpec("width_mm", value=30.0, min=10.0, max=100.0, unit='mm'),
        ParamSpec("height_mm", value=50.0, min=15.0, max=150.0, unit='mm'),
