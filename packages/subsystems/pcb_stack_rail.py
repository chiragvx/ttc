"""Pcb Stack Rail -- Rail supporting a vertical stack of PCBs
...
_FRAGMENT = """\
## Subsystem: Pcb Stack Rail
Rail supporting a vertical stack of PCBs -- an extruded U-channel, open at the top and both ends (FDM/FFF or extruded stock).
...
PCB_STACK_RAIL = register_subsystem(Subsystem(
    name="pcb_stack_rail",
    description="Rail supporting a vertical stack of PCBs -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=100.0, min=20.0, max=350.0, unit='mm'),
        ParamSpec("width_mm", value=12.0, min=4.0, max=50.0, unit='mm'),
        ParamSpec("height_mm", value=10.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("wall_mm", value=1.5, min=0.6, max=6.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
