"""Prop Hub Blank -- Prop hub spacer/blank (no blade geometry)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Prop Hub Blank
Prop hub spacer/blank (no blade geometry) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
PROP_HUB_BLANK = register_subsystem(Subsystem(
    name="prop_hub_blank",
    description="Prop hub spacer/blank (no blade geometry) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=20.0, min=8.0, max=70.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=5.0, min=1.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=10.0, min=2.0, max=40.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
