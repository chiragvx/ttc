"""Tailwheel Bracket -- Bracket mounting a small steerable tailwheel

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
4) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Tailwheel Bracket
Bracket mounting a small steerable tailwheel -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
TAILWHEEL_BRACKET = register_subsystem(Subsystem(
    name="tailwheel_bracket",
    description="Bracket mounting a small steerable tailwheel -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=25.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("leg_b_mm", value=30.0, min=10.0, max=90.0, unit='mm'),
        ParamSpec("width_mm", value=18.0, min=6.0, max=70.0, unit='mm'),
