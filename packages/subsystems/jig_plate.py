"""Jig Plate -- Plate with a grid of dowel-pin bores and mounting slots (dowel-pin grid approximated as a linear hole row)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
9) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Jig Plate
Plate with a grid of dowel-pin bores and mounting slots (dowel-pin grid approximated as a linear hole row) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
JIG_PLATE = register_subsystem(Subsystem(
    name="jig_plate",
    description="Plate with a grid of dowel-pin bores and mounting slots -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=80.0, min=20.0, max=280.0, unit='mm'),
        ParamSpec("depth_mm", value=60.0, min=15.0, max=220.0, unit='mm'),
        ParamSpec("thickness_mm", value=8.0, min=2.0, max=30.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=5.0, min=2.0, max=10.0, unit='mm'),
        ParamSpec("n_holes", value=6, min=0, max=12, unit='count'),
