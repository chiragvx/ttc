"""Wing Fold Hinge -- Static hinge leaf for ground-transport wing folding (geometry only, no deployment dynamics)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
2) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Wing Fold Hinge
Static hinge leaf for ground-transport wing folding (geometry only, no deployment dynamics) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
WING_FOLD_HINGE = register_subsystem(Subsystem(
    name="wing_fold_hinge",
    description="Static hinge leaf for ground-transport wing folding (geometry only, no deployment dynamics) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=50.0, min=15.0, max=150.0, unit='mm'),
        ParamSpec("depth_mm", value=25.0, min=8.0, max=100.0, unit='mm'),
        ParamSpec("thickness_mm", value=4.0, min=0.8, max=15.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=4.0, min=2.0, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=1, min=1, max=1, unit='count'),
        ParamSpec("margin_mm", value=8.0, min=3.0, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=plate_face_interfaces("thickness_mm"),  # 2026-07-27 — mounts via a flat face
))
