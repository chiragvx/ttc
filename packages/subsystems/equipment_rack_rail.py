"""Equipment Rack Rail -- Rail with a regular hole/slot pitch for stacking equipment

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
6) -- an extruded U-channel (base + two side walls, open top and both ends), the SAME
...
_FRAGMENT = """\
## Subsystem: Equipment Rack Rail
Rail with a regular hole/slot pitch for stacking equipment -- an extruded U-channel, open at the top and both ends (FDM/FFF or extruded stock).
...
EQUIPMENT_RACK_RAIL = register_subsystem(Subsystem(
    name="equipment_rack_rail",
    description="Rail with a regular hole/slot pitch for stacking equipment -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=150.0, min=30.0, max=500.0, unit='mm'),
        ParamSpec("width_mm", value=20.0, min=6.0, max=80.0, unit='mm'),
