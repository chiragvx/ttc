"""Breakout Plate -- Plate with a labelled hole cluster for connectors (USB, Ethernet, etc.) (keyed connector cutouts approximated as round holes)
...
_FRAGMENT = """\
## Subsystem: Breakout Plate
Plate with a labelled hole cluster for connectors (USB, Ethernet, etc.) (keyed connector cutouts approximated as round holes) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
BREAKOUT_PLATE = register_subsystem(Subsystem(
    name="breakout_plate",
    description="Plate with a labelled hole cluster for connectors (USB, Ethernet, etc.) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=55.0, min=15.0, max=200.0, unit='mm'),
        ParamSpec("depth_mm", value=30.0, min=8.0, max=120.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=10.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("n_holes", value=4, min=0, max=6, unit='count'),
        ParamSpec("margin_mm", value=8.0, min=3.0, max=25.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
