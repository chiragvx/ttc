"""Floor Flange -- Round disc + upright collar (e.g. rod-to-floor)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
2) -- a cylindrical shaft/boss with a wider mounting flange at its base, a concentric bore,
...
_FRAGMENT = """\
## Subsystem: Floor Flange
Round disc + upright collar (e.g. rod-to-floor) -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
FLOOR_FLANGE = register_subsystem(Subsystem(
    ...
    params=[
        ParamSpec("shaft_dia_mm", value=15.0, min=5.0, max=50.0, unit='mm'),
        ParamSpec("shaft_len_mm", value=20.0, min=5.0, max=70.0, unit='mm'),
        ParamSpec("flange_dia_mm", value=40.0, min=15.0, max=120.0, unit='mm'),
        ParamSpec("flange_thickness_mm", value=4.0, min=1.0, max=15.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=10.0, min=2.0, max=40.0, unit='mm'),
        ParamSpec("n_bolt_holes", value=4, min=0, max=8, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=4.0, min=1.5, max=10.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
