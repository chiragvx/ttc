"""Pulley Blank Timing -- Timing-pulley profile without teeth (smooth OD) (timing teeth not modeled)
...
_FRAGMENT = """\
## Subsystem: Pulley Blank Timing
Timing-pulley profile without teeth (smooth OD) (timing teeth not modeled) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
PULLEY_BLANK_TIMING = register_subsystem(Subsystem(
    name="pulley_blank_timing",
    description="Timing-pulley profile without teeth (smooth OD) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=30.0, min=10.0, max=100.0, unit='mm'),
        ParamSpec("height_mm", value=12.0, min=3.0, max=40.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    ...
    # 2026-07-28 -- a plain bd.Cylinder(radius, height), centered at the origin along local Z by
    # construction (identical shape/param convention to round_post.py, which this mirrors), so its two
    # flat end faces are exactly what cylinder_end_interfaces expects (e.g. mounting this blank onto a
    # shaft/hub end-to-end).
