"""Keyway Shaft -- Round shaft with a keyway slot (keyway slot not modeled)
...
_FRAGMENT = """\
## Subsystem: Keyway Shaft
Round shaft with a keyway slot (keyway slot not modeled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
KEYWAY_SHAFT = register_subsystem(Subsystem(
    name="keyway_shaft",
    description="Round shaft with a keyway slot -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=10.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=1.0, min=0.1, max=8.0, unit='mm'),
        ParamSpec("height_mm", value=60.0, min=10.0, max=250.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
