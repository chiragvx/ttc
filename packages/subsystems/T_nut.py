"""T Nut -- Flanged nut for wood/T-slot channels

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
1) -- a cylindrical shaft/boss with a wider mounting flange at its base, a concentric bore,
...
_FRAGMENT = """\
## Subsystem: T Nut
Flanged nut for wood/T-slot channels -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
def _flange_face(p):
    ...
    helpers). This is the wide face that seats flush against the workpiece/T-slot channel surface, so
    its outward normal points -Z (away from the part, into whatever it's mounted to)."""
...
T_NUT = register_subsystem(Subsystem(
    name="T_nut",
    description="Flanged nut for wood/T-slot channels -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("shaft_dia_mm", value=8.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("shaft_len_mm", value=8.0, min=2.0, max=30.0, unit='mm'),
        ParamSpec("flange_dia_mm", value=18.0, min=6.0, max=50.0, unit='mm'),
        ParamSpec("flange_thickness_mm", value=2.0, min=0.8, max=8.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=4.0, min=1.0, max=15.0, unit='mm'),
        ParamSpec("n_bolt_holes", value=0, min=0, max=6, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=2.0, min=1.0, max=6.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    ...
    # frames computed from the part's own real z-offsets instead. `flange_face` is the real mounting
    # face (seats against the T-slot/workpiece); `shaft_tip` is the boss's free end.
    interfaces=[
