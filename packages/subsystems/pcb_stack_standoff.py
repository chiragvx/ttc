"""Pcb Stack Standoff -- Standoff sized for a satellite PCB stack pitch

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
12) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Pcb Stack Standoff
Standoff sized for a satellite PCB stack pitch -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
PCB_STACK_STANDOFF = register_subsystem(Subsystem(
    name="pcb_stack_standoff",
    description="Standoff sized for a satellite PCB stack pitch -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=6.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=2.7, min=1.0, max=15.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=4.0, max=60.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    ...
    # fit_socket shape (bore receives `host_dia_mm + clearance_mm` from a round shaft/post/bolt host) —
    # a PCB stack standoff's through-bore is the same case, sized for a smaller board-mount screw.
