"""Fender Washer -- Very wide flat washer (larger OD ratio)
...
_FRAGMENT = """\
## Subsystem: Fender Washer
Very wide flat washer (larger OD ratio) -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
FENDER_WASHER = register_subsystem(Subsystem(
    name="fender_washer",
    description="Very wide flat washer (larger OD ratio) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=30.0, min=10.0, max=100.0, unit='mm'),
        ParamSpec("flange_width_mm", value=12.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("thickness_mm", value=1.5, min=0.6, max=6.0, unit='mm'),
        ParamSpec("num_bolt_holes", value=0, min=0, max=1, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=1.0, min=0.5, max=3.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
