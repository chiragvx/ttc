"""Data Port Bezel -- Bezel around an external data-port cutout
...
_FRAGMENT = """\
## Subsystem: Data Port Bezel
Bezel around an external data-port cutout -- a flat plate with a central cutout and four corner mounting holes (FDM/FFF or CNC).
...
DATA_PORT_BEZEL = register_subsystem(Subsystem(
    name="data_port_bezel",
    description="Bezel around an external data-port cutout -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=25.0, min=8.0, max=90.0, unit='mm'),
        ParamSpec("height_mm", value=18.0, min=6.0, max=70.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
        ParamSpec("window_w_mm", value=10.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("window_h_mm", value=6.0, min=2.0, max=30.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=2.0, min=1.0, max=5.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=3.0, min=1.0, max=10.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
