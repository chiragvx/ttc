"""Cable Tie Anchor -- Flat mount with a cable-tie pass-through slot (pass-through slot approximated as a round hole)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
1) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Cable Tie Anchor
Flat mount with a cable-tie pass-through slot (pass-through slot approximated as a round hole) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
CABLE_TIE_ANCHOR = register_subsystem(Subsystem(
    name="cable_tie_anchor",
    description="Flat mount with a cable-tie pass-through slot -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=18.0, min=6.0, max=60.0, unit='mm'),
        ParamSpec("depth_mm", value=12.0, min=4.0, max=40.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=1, min=0, max=2, unit='count'),
        ParamSpec("margin_mm", value=5.0, min=2.0, max=15.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
