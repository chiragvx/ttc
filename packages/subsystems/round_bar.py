"""Round Bar -- Solid round bar (long dowel; distinct from dowel_pin by intent)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
5) -- a plain solid cylinder. Fine surface/profile detail this part's real-world name
...
_FRAGMENT = """\
## Subsystem: Round Bar
Solid round bar (long dowel; distinct from dowel_pin by intent) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
ROUND_BAR = register_subsystem(Subsystem(
    name="round_bar",
    description="Solid round bar (long dowel; distinct from dowel_pin by intent) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=10.0, min=3.0, max=50.0, unit='mm'),
        ParamSpec("height_mm", value=300.0, min=30.0, max=1200.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
