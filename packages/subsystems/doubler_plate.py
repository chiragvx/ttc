"""Doubler Plate -- Local reinforcement plate over a stressed area
...
_FRAGMENT = """\
## Subsystem: Doubler Plate
Local reinforcement plate over a stressed area -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
DOUBLER_PLATE = register_subsystem(Subsystem(
    name="doubler_plate",
    description="Local reinforcement plate over a stressed area -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=60.0, min=15.0, max=200.0, unit='mm'),
        ParamSpec("depth_mm", value=40.0, min=10.0, max=150.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.0, min=0.8, max=10.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.4, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=2, min=0, max=8, unit='count'),
        ParamSpec("margin_mm", value=8.0, min=3.0, max=30.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
