"""Wheel Hub -- Wheel hub with axle bore
...
_FRAGMENT = """\
## Subsystem: Wheel Hub
Wheel hub with axle bore -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
WHEEL_HUB = register_subsystem(Subsystem(
    name="wheel_hub",
    description="Wheel hub with axle bore -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=30.0, min=10.0, max=100.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=6.0, min=2.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=12.0, min=3.0, max=50.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
