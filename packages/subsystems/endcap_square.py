"""Endcap Square -- Cap for a square tube OD (modeled open-ended rather than as a blind pocket cap)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
3) -- an extruded hollow square section, the SAME shape family `square_tube.py` already
...
_FRAGMENT = """\
## Subsystem: Endcap Square
Cap for a square tube OD (modeled open-ended rather than as a blind pocket cap) -- an extruded hollow square section (FDM/FFF or extruded stock).
...
ENDCAP_SQUARE = register_subsystem(Subsystem(
    name="endcap_square",
    description="Cap for a square tube OD -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=15.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("outer_side_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
