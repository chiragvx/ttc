"""Tail Skid -- Ground-contact skid at the tail
...
_FRAGMENT = """\
## Subsystem: Tail Skid
Ground-contact skid at the tail -- a solid rectangular bar (FDM/FFF or CNC).
...
TAIL_SKID = register_subsystem(Subsystem(
    name="tail_skid",
    description="Ground-contact skid at the tail -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    ...
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # same Box(length,width,height) bar shape family as flat_bar/longeron
))
