"""Wing Nut -- Hex bore + two winged tabs for hand-tightening (wing tabs not modeled, approximated as a plain boss)
...
_FRAGMENT = """\
## Subsystem: Wing Nut
Hex bore + two winged tabs for hand-tightening (wing tabs not modeled, approximated as a plain boss) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
WING_NUT = register_subsystem(Subsystem(
    name="wing_nut",
    description="Hex bore + two winged tabs for hand-tightening -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=12.0, min=5.0, max=40.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=3.4, min=1.0, max=25.0, unit='mm'),
