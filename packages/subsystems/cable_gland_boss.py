"""Cable Gland Boss -- Standalone gland-mounting boss (splices into an enclosure wall)
...
_FRAGMENT = """\
## Subsystem: Cable Gland Boss
Standalone gland-mounting boss (splices into an enclosure wall) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
CABLE_GLAND_BOSS = register_subsystem(Subsystem(
    name="cable_gland_boss",
    description="Standalone gland-mounting boss (splices into an enclosure wall) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=14.0, min=5.0, max=45.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=8.0, min=2.0, max=35.0, unit='mm'),
        ParamSpec("height_mm", value=8.0, min=2.0, max=25.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
