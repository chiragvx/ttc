"""Clamp Two Halves -- Generic bolted clamp: matching top + bottom half (represents one half; the matching half is a second instance)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
13) -- a mounting block with an OPEN semi-circular cradle channel + two base mounting
...
_FRAGMENT = """\
## Subsystem: Clamp Two Halves
Generic bolted clamp: matching top + bottom half (represents one half; the matching half is a second instance) -- a mounting block with an OPEN semi-circular cradle channel (cradles a cylindrical
...
CLAMP_TWO_HALVES = register_subsystem(Subsystem(
    ...
    params=[
        ParamSpec("length_mm", value=25.0, min=8.0, max=90.0, unit='mm'),
        ParamSpec("width_mm", value=70.0, min=15.0, max=140.0, unit='mm'),
        ParamSpec("height_mm", value=40.0, min=10.0, max=90.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=20.0, min=5.0, max=120.0, unit='mm'),
