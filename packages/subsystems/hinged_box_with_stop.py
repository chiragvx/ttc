"""Hinged Box With Stop -- Hinged box + integral doorstop for the lid (hinge knuckles and doorstop not modeled)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
13) -- an open-top rectangular shell, the SAME shape family `enclosure.py` already
...
_FRAGMENT = """\
## Subsystem: Hinged Box With Stop
Hinged box + integral doorstop for the lid (hinge knuckles and doorstop not modeled) -- an open-top rectangular shell (FDM/FFF), a floor of thickness wall_mm, open at the
...
HINGED_BOX_WITH_STOP = register_subsystem(Subsystem(
    name="hinged_box_with_stop",
    description="Hinged box + integral doorstop for the lid -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=75.0, min=18.0, max=240.0, unit='mm'),
        ParamSpec("depth_mm", value=55.0, min=14.0, max=190.0, unit='mm'),
        ParamSpec("height_mm", value=38.0, min=10.0, max=130.0, unit='mm'),
        ParamSpec("wall_mm", value=2.0, min=0.8, max=8.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
