"""Deployment Hinge -- Static two-leaf hinge leaf for solar panels/deployables (geometry only, no deployment dynamics)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
10) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Deployment Hinge
Static two-leaf hinge leaf for solar panels/deployables (geometry only, no deployment dynamics) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
DEPLOYMENT_HINGE = register_subsystem(Subsystem(
    name="deployment_hinge",
    description="Static two-leaf hinge leaf for solar panels/deployables (geometry only, no deployment dynamics) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=35.0, min=12.0, max=120.0, unit='mm'),
        ParamSpec("depth_mm", value=18.0, min=6.0, max=70.0, unit='mm'),
        ...
        ParamSpec("margin_mm", value=6.0, min=2.0, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
