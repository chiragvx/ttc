"""Solar Panel Backing Plate -- Backing plate a solar cell array mounts to

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
12) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Solar Panel Backing Plate
Backing plate a solar cell array mounts to -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
SOLAR_PANEL_BACKING_PLATE = register_subsystem(Subsystem(
    name="solar_panel_backing_plate",
    description="Backing plate a solar cell array mounts to -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=85.0, min=20.0, max=300.0, unit='mm'),
        ParamSpec("depth_mm", value=85.0, min=20.0, max=300.0, unit='mm'),
        ParamSpec("thickness_mm", value=1.5, min=0.6, max=8.0, unit='mm'),
