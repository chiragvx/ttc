"""Fairing Ring -- Ring frame for a nose-cone/fairing base (the ring only, not the aero skin)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
14) -- a flat ring frame + evenly-spaced bolt-hole pattern, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Fairing Ring
Ring frame for a nose-cone/fairing base (the ring only, not the aero skin) -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
FAIRING_RING = register_subsystem(Subsystem(
    name="fairing_ring",
    description="Ring frame for a nose-cone/fairing base (the ring only, not the aero skin) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=95.0, min=30.0, max=320.0, unit='mm'),
        ParamSpec("flange_width_mm", value=7.0, min=2.0, max=25.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.0, min=0.8, max=8.0, unit='mm'),
        ParamSpec("num_bolt_holes", value=6, min=3, max=16, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=3.0, min=1.5, max=6.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
