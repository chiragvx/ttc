"""Bezel Display -- Rectangular frame for a display window
...
_FRAGMENT = """\
## Subsystem: Bezel Display
Rectangular frame for a display window -- a flat plate with a central cutout and four corner mounting holes (FDM/FFF or CNC).
...
BEZEL_DISPLAY = register_subsystem(Subsystem(
    name="bezel_display",
    description="Rectangular frame for a display window -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=60.0, min=15.0, max=220.0, unit='mm'),
        ParamSpec("height_mm", value=40.0, min=10.0, max=160.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
        ParamSpec("window_w_mm", value=45.0, min=8.0, max=200.0, unit='mm'),
        ParamSpec("window_h_mm", value=28.0, min=6.0, max=140.0, unit='mm'),
