"""Socket Cap Bolt Blank -- Cylindrical head with hex socket (hex socket/thread not modeled)
...
_FRAGMENT = """\
## Subsystem: Socket Cap Bolt Blank
Cylindrical head with hex socket (hex socket/thread not modeled) -- a two-diameter stepped/shouldered cylinder, optional through-bore.
...
SOCKET_CAP_BOLT_BLANK = register_subsystem(Subsystem(
    name="socket_cap_bolt_blank",
    description="Cylindrical head with hex socket -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia1_mm", value=8.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("dia2_mm", value=4.0, min=1.5, max=18.0, unit='mm'),
        ParamSpec("len1_mm", value=5.0, min=2.0, max=15.0, unit='mm'),
    ...
    # z-offsets instead, same pattern as `hex_bolt_blank`/`button_head_bolt_blank`/`T_nut`'s
    # flange/shaft. `head_face` is the head's real bearing face; `tip_face` is the shank's free end.
    interfaces=[
