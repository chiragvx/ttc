"""Oring Boss -- Cylindrical boss with an O-ring groove (O-ring groove not modeled)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
10) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Oring Boss
Cylindrical boss with an O-ring groove (O-ring groove not modeled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
ORING_BOSS = register_subsystem(Subsystem(
    name="oring_boss",
    description="Cylindrical boss with an O-ring groove -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=18.0, min=6.0, max=60.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=10.0, min=2.0, max=50.0, unit='mm'),
