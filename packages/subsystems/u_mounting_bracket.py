"""U Mounting Bracket -- U-shape (two vertical walls, base) -- short mounting-focused

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
2) -- an extruded U-channel (base + two side walls, open top and both ends), the SAME
...
_FRAGMENT = """\
## Subsystem: U Mounting Bracket
U-shape (two vertical walls, base) -- short mounting-focused -- an extruded U-channel, open at the top and both ends (FDM/FFF or extruded stock).
...
U_MOUNTING_BRACKET = register_subsystem(Subsystem(
    name="u_mounting_bracket",
    description="U-shape (two vertical walls, base) -- short mounting-focused -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=35.0, min=10.0, max=120.0, unit='mm'),
        ParamSpec("width_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
        ParamSpec("height_mm", value=18.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("wall_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
