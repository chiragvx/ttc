"""Parallel Block Pair -- Precision parallel blocks (matched pair) (represents one block; the matched pair is a second instance)
...
_FRAGMENT = """\
## Subsystem: Parallel Block Pair
Precision parallel blocks (matched pair) (represents one block; the matched pair is a second instance) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
PARALLEL_BLOCK_PAIR = register_subsystem(Subsystem(
    name="parallel_block_pair",
    description="Precision parallel blocks (matched pair) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=60.0, min=15.0, max=200.0, unit='mm'),
        ParamSpec("depth_mm", value=15.0, min=5.0, max=50.0, unit='mm'),
        ParamSpec("thickness_mm", value=10.0, min=3.0, max=35.0, unit='mm'),
