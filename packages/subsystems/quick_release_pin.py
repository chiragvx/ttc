"""Quick Release Pin -- Spring-loaded quick-release pin (mechanism not modelled -- geometry only)
...
_FRAGMENT = """\
## Subsystem: Quick Release Pin
Spring-loaded quick-release pin (mechanism not modelled -- geometry only) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
QUICK_RELEASE_PIN = register_subsystem(Subsystem(
    name="quick_release_pin",
    description="Spring-loaded quick-release pin (mechanism not modelled -- geometry only) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=5.0, min=2.0, max=20.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=1.0, min=0.1, max=10.0, unit='mm'),
        ParamSpec("height_mm", value=30.0, min=8.0, max=100.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
