"""Tri Stand -- Tripod: top plate + 3 legs (represents one leg; the top plate and other legs are separate instances)
...
_FRAGMENT = """\
## Subsystem: Tri Stand
Tripod: top plate + 3 legs (represents one leg; the top plate and other legs are separate instances) -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
TRI_STAND = register_subsystem(Subsystem(
    ...
    params=[
        ...
        ParamSpec("shaft_len_mm", value=150.0, min=30.0, max=500.0, unit='mm'),
        ParamSpec("flange_dia_mm", value=30.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("flange_thickness_mm", value=4.0, min=1.0, max=15.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=3.4, min=1.0, max=20.0, unit='mm'),
        ParamSpec("n_bolt_holes", value=0, min=0, max=4, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=3.0, min=1.0, max=8.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
