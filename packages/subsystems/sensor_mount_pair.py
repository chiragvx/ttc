"""Sensor Mount Pair -- Two matched sensor mounts (e.g. IR emitter/receiver) (represents one mount; the matched pair is a second instance)
...
_FRAGMENT = """\
## Subsystem: Sensor Mount Pair
Two matched sensor mounts (e.g. IR emitter/receiver) (represents one mount; the matched pair is a second instance) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
SENSOR_MOUNT_PAIR = register_subsystem(Subsystem(
    name="sensor_mount_pair",
    description="Two matched sensor mounts (e.g. IR emitter/receiver) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=25.0, min=8.0, max=90.0, unit='mm'),
        ParamSpec("depth_mm", value=20.0, min=6.0, max=75.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=6.0, unit='mm'),
        ParamSpec("n_holes", value=2, min=0, max=4, unit='count'),
