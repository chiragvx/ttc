"""Zip Tie Saddle -- Small mount with a zip-tie pass-through (pass-through slot approximated as a round hole)
...
_FRAGMENT = """\
## Subsystem: Zip Tie Saddle
Small mount with a zip-tie pass-through (pass-through slot approximated as a round hole) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
ZIP_TIE_SADDLE = register_subsystem(Subsystem(
    name="zip_tie_saddle",
    description="Small mount with a zip-tie pass-through -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=15.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("depth_mm", value=12.0, min=4.0, max=45.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=6.0, unit='mm'),
        ParamSpec("n_holes", value=1, min=1, max=2, unit='count'),
        ParamSpec("margin_mm", value=4.0, min=1.5, max=12.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
