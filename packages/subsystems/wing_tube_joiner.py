"""Wing Tube Joiner -- Tube-and-sleeve wing joiner (common in foam-board builds)
...
_FRAGMENT = """\
## Subsystem: Wing Tube Joiner
Tube-and-sleeve wing joiner (common in foam-board builds) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
WING_TUBE_JOINER = register_subsystem(Subsystem(
    name="wing_tube_joiner",
    description="Tube-and-sleeve wing joiner (common in foam-board builds) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=16.0, min=6.0, max=50.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=10.0, min=3.0, max=45.0, unit='mm'),
        ParamSpec("height_mm", value=50.0, min=10.0, max=180.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
