"""Split Shell Case -- Two mirror halves that clamshell together (one half; the mirrored half is a second instance)
...
_FRAGMENT = """\
## Subsystem: Split Shell Case
Two mirror halves that clamshell together (one half; the mirrored half is a second instance) -- an open-top rectangular shell (FDM/FFF), a floor of thickness wall_mm, open at the
...
SPLIT_SHELL_CASE = register_subsystem(Subsystem(
    name="split_shell_case",
    description="Two mirror halves that clamshell together -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=80.0, min=20.0, max=250.0, unit='mm'),
        ParamSpec("depth_mm", value=60.0, min=15.0, max=200.0, unit='mm'),
        ParamSpec("height_mm", value=35.0, min=10.0, max=120.0, unit='mm'),
