"""Control Horn -- Horn arm a pushrod attaches to on a control surface

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
8) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Control Horn
Horn arm a pushrod attaches to on a control surface -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
CONTROL_HORN = register_subsystem(Subsystem(
    name="control_horn",
    description="Horn arm a pushrod attaches to on a control surface -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
