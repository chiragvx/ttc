"""Bar Pull -- Straight-bar drawer pull
...
_FRAGMENT = """\
## Subsystem: Bar Pull
Straight-bar drawer pull -- a solid rectangular bar (FDM/FFF or CNC).
...
BAR_PULL = register_subsystem(Subsystem(
    name="bar_pull",
    description="Straight-bar drawer pull -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=120.0, min=25.0, max=350.0, unit='mm'),
        ParamSpec("width_mm", value=10.0, min=3.0, max=35.0, unit='mm'),
        ParamSpec("height_mm", value=10.0, min=3.0, max=35.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # bd.Box(length_mm, width_mm, height_mm), centered — same shape family as flat_bar/longeron
))
