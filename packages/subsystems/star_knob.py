"""Star Knob -- Star-shape (n lobes) hand knob (lobes not modeled, approximated as a round knob)
...
_FRAGMENT = """\
## Subsystem: Star Knob
Star-shape (n lobes) hand knob (lobes not modeled, approximated as a round knob) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
STAR_KNOB = register_subsystem(Subsystem(
    name="star_knob",
    description="Star-shape (n lobes) hand knob -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=35.0, min=10.0, max=100.0, unit='mm'),
    ...
    # 2026-07-28 -- plain bd.Cylinder(radius=p.dia_mm/2.0, height=p.height_mm), centered at the origin
    # along local Z by build123d default (same shape family as round_post/round_bar, confirmed by
    # reading _build above) -- its two flat end faces are exactly what cylinder_end_interfaces expects.
