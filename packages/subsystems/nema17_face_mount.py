"""Nema17 Face Mount -- Motor mount with the NEMA17 hole pattern preset

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
2) -- a flat plate with a central round bore (a shaft/spindle clearance) plus four corner
...
_FRAGMENT = """\
## Subsystem: Nema17 Face Mount
Motor mount with the NEMA17 hole pattern preset -- a flat plate with a central round bore and four corner mounting holes.
...
NEMA17_FACE_MOUNT = register_subsystem(Subsystem(
    name="nema17_face_mount",
    description="Motor mount with the NEMA17 hole pattern preset -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=42.3, min=20.0, max=100.0, unit='mm'),
        ParamSpec("depth_mm", value=42.3, min=20.0, max=100.0, unit='mm'),
        ParamSpec("thickness_mm", value=4.0, min=1.0, max=15.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=22.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.2, min=1.5, max=6.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=5.15, min=2.0, max=15.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=plate_face_interfaces("thickness_mm"),  # 2026-07-27 — plain width x depth x thickness
    # Box centered at the origin (bore + 4 corner holes are inset cuts, same shape family as `panel`).
))
