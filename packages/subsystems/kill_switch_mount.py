"""Kill Switch Mount -- Mount for a remove-before-flight kill switch
...
_FRAGMENT = """\
## Subsystem: Kill Switch Mount
Mount for a remove-before-flight kill switch -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
KILL_SWITCH_MOUNT = register_subsystem(Subsystem(
    name="kill_switch_mount",
    description="Mount for a remove-before-flight kill switch -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
