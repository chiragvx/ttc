"""Handle Plate -- Plate + integrated pull handle (integrated handle relief not modeled)
...
_FRAGMENT = """\
## Subsystem: Handle Plate
Plate + integrated pull handle (integrated handle relief not modeled) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
HANDLE_PLATE = register_subsystem(Subsystem(
    name="handle_plate",
    description="Plate + integrated pull handle -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=60.0, min=15.0, max=220.0, unit='mm'),
        ParamSpec("depth_mm", value=35.0, min=10.0, max=130.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.5, min=0.8, max=14.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=4.0, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=4, min=0, max=8, unit='count'),
        ParamSpec("margin_mm", value=7.0, min=2.0, max=25.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
