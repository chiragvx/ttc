"""Wire Clip -- Small clip that holds one or two round conductors (conductor cradle shape simplified)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
12) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Wire Clip
Small clip that holds one or two round conductors (conductor cradle shape simplified) -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
WIRE_CLIP = register_subsystem(Subsystem(
    name="wire_clip",
    description="Small clip that holds one or two round conductors -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=10.0, min=4.0, max=35.0, unit='mm'),
        ParamSpec("leg_b_mm", value=10.0, min=4.0, max=35.0, unit='mm'),
        ParamSpec("width_mm", value=8.0, min=3.0, max=30.0, unit='mm'),
