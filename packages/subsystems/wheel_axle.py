...
_FRAGMENT = """\
## Subsystem: Wheel Axle
Axle pin -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
WHEEL_AXLE = register_subsystem(Subsystem(
    name="wheel_axle",
    description="Axle pin -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=6.0, min=2.0, max=25.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=1.0, min=0.1, max=10.0, unit='mm'),
        ParamSpec("height_mm", value=50.0, min=10.0, max=200.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
