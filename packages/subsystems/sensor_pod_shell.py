"""Sensor Pod Shell -- Shell housing for an external sensor pod

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
6) -- an open-top rectangular shell, the SAME shape family `enclosure.py` already
...
_FRAGMENT = """\
## Subsystem: Sensor Pod Shell
Shell housing for an external sensor pod -- an open-top rectangular shell (FDM/FFF), a floor of thickness wall_mm, open at the
...
SENSOR_POD_SHELL = register_subsystem(Subsystem(
    name="sensor_pod_shell",
    description="Shell housing for an external sensor pod -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
