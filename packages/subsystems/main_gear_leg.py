"""Main Gear Leg -- Main landing-gear leg (straight or bowed strut)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
4) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Main Gear Leg
Main landing-gear leg (straight or bowed strut) -- a solid rectangular bar (FDM/FFF or CNC).
...
MAIN_GEAR_LEG = register_subsystem(Subsystem(
    name="main_gear_leg",
    description="Main landing-gear leg (straight or bowed strut) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=180.0, min=40.0, max=600.0, unit='mm'),
        ParamSpec("width_mm", value=12.0, min=4.0, max=60.0, unit='mm'),
        ParamSpec("height_mm", value=8.0, min=2.0, max=40.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
