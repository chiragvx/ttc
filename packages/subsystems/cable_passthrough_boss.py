"""Cable Passthrough Boss -- Boss with a grommet bore for a bulkhead cable pass-through

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
6) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Cable Passthrough Boss
Boss with a grommet bore for a bulkhead cable pass-through -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
CABLE_PASSTHROUGH_BOSS = register_subsystem(Subsystem(
    name="cable_passthrough_boss",
    description="Boss with a grommet bore for a bulkhead cable pass-through -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=12.0, min=5.0, max=40.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=6.0, min=2.0, max=30.0, unit='mm'),
