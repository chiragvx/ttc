"""Pcb Carrier -- Plate with 4 boss standoffs sized to a PCB hole pattern (modeled as a mounting plate with 4 corner holes; raised boss towers not separately modeled)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
4) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Pcb Carrier
Plate with 4 boss standoffs sized to a PCB hole pattern (modeled as a mounting plate with 4 corner holes; raised boss towers not separately modeled) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
PCB_CARRIER = register_subsystem(Subsystem(
    name="pcb_carrier",
    description="Plate with 4 boss standoffs sized to a PCB hole pattern -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=65.0, min=18.0, max=220.0, unit='mm'),
        ParamSpec("depth_mm", value=45.0, min=12.0, max=180.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=6.0, unit='mm'),
        ParamSpec("n_holes", value=4, min=0, max=8, unit='count'),
        ParamSpec("margin_mm", value=7.0, min=2.0, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
