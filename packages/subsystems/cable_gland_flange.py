"""Cable Gland Flange -- Flanged gland (compound with retainer nut) (retainer nut is a separate instance; thread not modeled)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
12) -- a cylindrical shaft/boss with a wider mounting flange at its base, a concentric bore,
...
_FRAGMENT = """\
## Subsystem: Cable Gland Flange
Flanged gland (compound with retainer nut) (retainer nut is a separate instance; thread not modeled) -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
def _flange_face(p):
    ...
    helpers). This is the wide face that seats flush against the enclosure wall the gland passes
    through, so its outward normal points -Z (away from the part, into whatever it's mounted to)."""
...
CABLE_GLAND_FLANGE = register_subsystem(Subsystem(
    name="cable_gland_flange",
    description="Flanged gland (compound with retainer nut) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("shaft_dia_mm", value=12.0, min=4.0, max=40.0, unit='mm'),
    ...
    # face (seats against the enclosure wall); `shaft_tip` is the boss's free end (where the gland's
    # retainer nut/cable enters).
    interfaces=[
