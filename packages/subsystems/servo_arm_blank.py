"""Servo Arm Blank -- Servo output arm blank

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
8) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Servo Arm Blank
Servo output arm blank -- a solid rectangular bar (FDM/FFF or CNC).
...
SERVO_ARM_BLANK = register_subsystem(Subsystem(
    name="servo_arm_blank",
    description="Servo output arm blank -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
        ParamSpec("width_mm", value=8.0, min=3.0, max=30.0, unit='mm'),
        ParamSpec("height_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # bd.Box(length,width,height), centered at origin along X
))
