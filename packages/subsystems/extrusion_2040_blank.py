"""2040 Extrusion Blank -- 20mm x 40mm T-slot extrusion (modeled as a square profile approximating the 20x40 envelope; T-slot channel not modeled)
...
_FRAGMENT = """\
## Subsystem: 2040 Extrusion Blank
20mm x 40mm T-slot extrusion (modeled as a square profile approximating the 20x40 envelope; T-slot channel not modeled) -- an extruded hollow square section (FDM/FFF or extruded stock).
...
EXTRUSION_2040_BLANK = register_subsystem(Subsystem(
    name="extrusion_2040_blank",
    description="20mm x 40mm T-slot extrusion -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=200.0, min=20.0, max=1000.0, unit='mm'),
        ParamSpec("outer_side_mm", value=30.0, min=18.0, max=42.0, unit='mm'),
        ParamSpec("wall_thickness_mm", value=2.0, min=0.8, max=8.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # Box(length_mm, outer_side_mm, outer_side_mm)
    # centered at the origin along local X, same bar-family convention as flat_bar/longeron.
))
