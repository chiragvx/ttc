"""Sleeve Bushing -- Simple cylindrical bushing (OD/ID/length)
...
_FRAGMENT = """\
## Subsystem: Sleeve Bushing
Simple cylindrical bushing (OD/ID/length) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
SLEEVE_BUSHING = register_subsystem(Subsystem(
    name="sleeve_bushing",
    description="Simple cylindrical bushing (OD/ID/length) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=12.0, min=4.0, max=40.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=8.0, min=2.0, max=35.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=3.0, max=50.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
