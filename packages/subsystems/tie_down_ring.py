"""Tie Down Ring -- Ground tie-down ring/eyelet mount

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
4) -- a flat ring frame + evenly-spaced bolt-hole pattern, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Tie Down Ring
Ground tie-down ring/eyelet mount -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
TIE_DOWN_RING = register_subsystem(Subsystem(
    name="tie_down_ring",
    description="Ground tie-down ring/eyelet mount -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=25.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("flange_width_mm", value=8.0, min=2.0, max=20.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=10.0, unit='mm'),
        ParamSpec("num_bolt_holes", value=3, min=2, max=8, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=3.4, min=1.5, max=6.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
