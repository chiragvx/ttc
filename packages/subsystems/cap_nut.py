"""Cap Nut -- Hex nut with a domed closed end (dome and hex flats not modeled)
...
_FRAGMENT = """\
## Subsystem: Cap Nut
Hex nut with a domed closed end (dome and hex flats not modeled) -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
CAP_NUT = register_subsystem(Subsystem(
    name="cap_nut",
    description="Hex nut with a domed closed end -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=10.0, min=4.0, max=35.0, unit='mm'),
