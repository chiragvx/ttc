"""Canopy Frame -- Frame for a canopy/window hatch
...
_FRAGMENT = """\
## Subsystem: Canopy Frame
Frame for a canopy/window hatch -- a flat plate with a central cutout and four corner mounting holes (FDM/FFF or CNC).
...
CANOPY_FRAME = register_subsystem(Subsystem(
    name="canopy_frame",
    description="Frame for a canopy/window hatch -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=90.0, min=25.0, max=350.0, unit='mm'),
        ParamSpec("height_mm", value=60.0, min=15.0, max=280.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=1.0, max=10.0, unit='mm'),
        ParamSpec("window_w_mm", value=65.0, min=8.0, max=320.0, unit='mm'),
        ParamSpec("window_h_mm", value=40.0, min=8.0, max=230.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.4, min=2.0, max=8.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=6.0, min=3.0, max=25.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
