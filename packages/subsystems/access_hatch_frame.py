"""Access Hatch Frame -- Frame around a fuselage cutout for a hatch

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
1) -- a flat plate with a central rectangular window/cutout plus four corner mounting
...
_FRAGMENT = """\
## Subsystem: Access Hatch Frame
Frame around a fuselage cutout for a hatch -- a flat plate with a central cutout and four corner mounting holes (FDM/FFF or CNC).
...
ACCESS_HATCH_FRAME = register_subsystem(Subsystem(
    name="access_hatch_frame",
    description="Frame around a fuselage cutout for a hatch -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=100.0, min=30.0, max=400.0, unit='mm'),
        ParamSpec("height_mm", value=80.0, min=20.0, max=300.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=1.0, max=10.0, unit='mm'),
        ParamSpec("window_w_mm", value=70.0, min=10.0, max=350.0, unit='mm'),
        ParamSpec("window_h_mm", value=50.0, min=10.0, max=250.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=4.0, min=2.0, max=8.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=8.0, min=3.0, max=30.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
