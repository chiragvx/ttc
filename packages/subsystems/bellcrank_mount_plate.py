"""Bellcrank Mount Plate -- Plate mounting a bellcrank pivot

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
8) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Bellcrank Mount Plate
Plate mounting a bellcrank pivot -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
BELLCRANK_MOUNT_PLATE = register_subsystem(Subsystem(
    name="bellcrank_mount_plate",
    description="Plate mounting a bellcrank pivot -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    ...
    params=[
        ...
        ParamSpec("n_holes", value=3, min=0, max=6, unit='count'),
