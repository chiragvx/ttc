"""Separation Ring -- Ring for staged/droppable payload separation

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
10) -- a flat ring frame + evenly-spaced bolt-hole pattern, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Separation Ring
Ring for staged/droppable payload separation -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
SEPARATION_RING = register_subsystem(Subsystem(
    name="separation_ring",
    description="Ring for staged/droppable payload separation -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=80.0, min=25.0, max=280.0, unit='mm'),
        ParamSpec("flange_width_mm", value=6.0, min=2.0, max=20.0, unit='mm'),
