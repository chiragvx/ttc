...
_FRAGMENT = """\
## Subsystem: Nema23 Face Mount
Same, NEMA23 -- a flat plate with a central round bore and four corner mounting holes.
...
NEMA23_FACE_MOUNT = register_subsystem(Subsystem(
    name="nema23_face_mount",
    description="Same, NEMA23 -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=56.4, min=25.0, max=130.0, unit='mm'),
        ParamSpec("depth_mm", value=56.4, min=25.0, max=130.0, unit='mm'),
        ParamSpec("thickness_mm", value=5.0, min=1.0, max=18.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=38.1, min=8.0, max=80.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=5.2, min=2.0, max=8.0, unit='mm'),
        ParamSpec("hole_margin_mm", value=7.9, min=3.0, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=plate_face_interfaces("thickness_mm"),  # 2026-07-28 — flat plate centered at origin,
    # thickness along local Z (bore + 4 corner holes are inset cuts, same shape family as panel.py)
))
