"""Stackable Bin -- Base + tapered walls sized so bins nest and stack (nesting taper not modeled)
...
_FRAGMENT = """\
## Subsystem: Stackable Bin
Base + tapered walls sized so bins nest and stack (nesting taper not modeled) -- an open-top rectangular shell (FDM/FFF), a floor of thickness wall_mm, open at the
...
STACKABLE_BIN = register_subsystem(Subsystem(
    name="stackable_bin",
    description="Base + tapered walls sized so bins nest and stack -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=90.0, min=20.0, max=280.0, unit='mm'),
        ParamSpec("depth_mm", value=70.0, min=15.0, max=220.0, unit='mm'),
        ParamSpec("height_mm", value=45.0, min=10.0, max=150.0, unit='mm'),
