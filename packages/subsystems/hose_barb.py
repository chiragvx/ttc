"""Hose Barb -- Hose-barb fitting (ridged cylinder + optional flange) (ridges not modeled)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
12) -- a two-diameter stepped/shouldered cylinder with an optional through-bore (bore_dia_mm
...
_FRAGMENT = """\
## Subsystem: Hose Barb
Hose-barb fitting (ridged cylinder + optional flange) (ridges not modeled) -- a two-diameter stepped/shouldered cylinder, optional through-bore.
...
HOSE_BARB = register_subsystem(Subsystem(
    ...
    params=[
        ParamSpec("dia1_mm", value=10.0, min=3.0, max=35.0, unit='mm'),
        ParamSpec("dia2_mm", value=8.0, min=2.0, max=30.0, unit='mm'),
        ParamSpec("len1_mm", value=6.0, min=2.0, max=20.0, unit='mm'),
        ParamSpec("len2_mm", value=18.0, min=4.0, max=55.0, unit='mm'),
