"""Set Screw Pocket -- A block with a threaded-boss pocket for a set screw at 90 deg (thread not modeled)
...
_FRAGMENT = """\
## Subsystem: Set Screw Pocket
A block with a threaded-boss pocket for a set screw at 90 deg (thread not modeled) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
SET_SCREW_POCKET = register_subsystem(Subsystem(
    name="set_screw_pocket",
    description="A block with a threaded-boss pocket for a set screw at 90 deg -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=25.0, min=10.0, max=80.0, unit='mm'),
        ParamSpec("depth_mm", value=20.0, min=8.0, max=70.0, unit='mm'),
        ParamSpec("thickness_mm", value=12.0, min=4.0, max=40.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.4, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=1, min=1, max=1, unit='count'),
        ParamSpec("margin_mm", value=10.0, min=4.0, max=30.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
