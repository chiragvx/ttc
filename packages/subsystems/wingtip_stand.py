"""Wingtip Stand -- Small stand supporting a wingtip on the ground

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
11) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Wingtip Stand
Small stand supporting a wingtip on the ground -- a solid rectangular bar (FDM/FFF or CNC).
...
WINGTIP_STAND = register_subsystem(Subsystem(
    name="wingtip_stand",
    description="Small stand supporting a wingtip on the ground -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=60.0, min=15.0, max=250.0, unit='mm'),
        ParamSpec("width_mm", value=15.0, min=5.0, max=60.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=5.0, max=60.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # same Box(length,width,height) family as flat_bar/longeron
))
