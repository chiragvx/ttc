"""Perforated Plate -- Plate with a dense hole pattern for airflow / grille (modeled as a single row of holes, not a full 2D grille pattern)
...
_FRAGMENT = """\
## Subsystem: Perforated Plate
Plate with a dense hole pattern for airflow / grille (modeled as a single row of holes, not a full 2D grille pattern) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
PERFORATED_PLATE = register_subsystem(Subsystem(
    name="perforated_plate",
    description="Plate with a dense hole pattern for airflow / grille -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=70.0, min=20.0, max=250.0, unit='mm'),
        ParamSpec("depth_mm", value=50.0, min=12.0, max=200.0, unit='mm'),
        ParamSpec("thickness_mm", value=2.5, min=0.8, max=10.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=4.0, min=1.5, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=8, min=3, max=20, unit='count'),
