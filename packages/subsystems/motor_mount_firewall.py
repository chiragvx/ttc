"""Motor Mount Firewall -- Bulkhead plate + bolt-circle for a motor, beefier than motor_mount

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Motor Mount Firewall
Bulkhead plate + bolt-circle for a motor, beefier than motor_mount -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
MOTOR_MOUNT_FIREWALL = register_subsystem(Subsystem(
    name="motor_mount_firewall",
    description="Bulkhead plate + bolt-circle for a motor, beefier than motor_mount -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=80.0, min=20.0, max=250.0, unit='mm'),
        ParamSpec("depth_mm", value=80.0, min=20.0, max=250.0, unit='mm'),
