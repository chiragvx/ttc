"""Nacelle Ring -- Ring frame forming a nacelle's structural core

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- a flat ring frame + evenly-spaced bolt-hole pattern, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Nacelle Ring
Ring frame forming a nacelle's structural core -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
NACELLE_RING = register_subsystem(Subsystem(
    name="nacelle_ring",
    description="Ring frame forming a nacelle's structural core -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=90.0, min=30.0, max=300.0, unit='mm'),
        ParamSpec("flange_width_mm", value=8.0, min=3.0, max=25.0, unit='mm'),
