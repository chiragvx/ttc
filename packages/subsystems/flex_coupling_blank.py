"""Flex Coupling Blank -- Solid cylindrical coupling with slit relief (slit relief not modeled)
...
_FRAGMENT = """\
## Subsystem: Flex Coupling Blank
Solid cylindrical coupling with slit relief (slit relief not modeled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
FLEX_COUPLING_BLANK = register_subsystem(Subsystem(
    name="flex_coupling_blank",
    description="Solid cylindrical coupling with slit relief -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=16.0, min=5.0, max=55.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=6.0, min=1.5, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=30.0, min=8.0, max=90.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
