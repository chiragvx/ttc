"""Motor Bracket Stack -- Motor mount + backing bracket (represents the backing bracket; motor_mount is a separate existing subsystem)
...
_FRAGMENT = """\
## Subsystem: Motor Bracket Stack
Motor mount + backing bracket (represents the backing bracket; motor_mount is a separate existing subsystem) -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
MOTOR_BRACKET_STACK = register_subsystem(Subsystem(
    name="motor_bracket_stack",
    description="Motor mount + backing bracket -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=30.0, min=12.0, max=90.0, unit='mm'),
        ParamSpec("leg_b_mm", value=30.0, min=12.0, max=90.0, unit='mm'),
