"""Power Distribution Mount Plate -- Plate mounting a PDB

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
7) -- a flat mounting plate with a row of bolt holes, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Power Distribution Mount Plate
Plate mounting a PDB -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
POWER_DISTRIBUTION_MOUNT_PLATE = register_subsystem(Subsystem(
    name="power_distribution_mount_plate",
    description="Plate mounting a PDB -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=50.0, min=15.0, max=180.0, unit='mm'),
        ParamSpec("depth_mm", value=40.0, min=12.0, max=150.0, unit='mm'),
