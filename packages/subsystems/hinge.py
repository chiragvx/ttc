"""Hinge -- Two mating leaves that pivot around a shared pin bore (one leaf; the mating leaf is a second instance, interlocking knuckles not modeled)
...
_FRAGMENT = """\
## Subsystem: Hinge
Two mating leaves that pivot around a shared pin bore (one leaf; the mating leaf is a second instance, interlocking knuckles not modeled) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
HINGE = register_subsystem(Subsystem(
    name="hinge",
    description="Two mating leaves that pivot around a shared pin bore -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=40.0, min=12.0, max=140.0, unit='mm'),
        ParamSpec("depth_mm", value=20.0, min=6.0, max=80.0, unit='mm'),
        ParamSpec("thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.5, max=8.0, unit='mm'),
