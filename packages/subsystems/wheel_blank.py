"""Wheel Blank -- Disc + hub, wide radial rim
...
_FRAGMENT = """\
## Subsystem: Wheel Blank
Disc + hub, wide radial rim -- a solid cylindrical body (FDM/FFF or turned). Fine surface detail (knurling, a domed
...
WHEEL_BLANK = register_subsystem(Subsystem(
    name="wheel_blank",
    description="Disc + hub, wide radial rim -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia_mm", value=60.0, min=15.0, max=200.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=4.0, max=50.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    ...
    # 2026-07-28 -- a plain bd.Cylinder(radius=dia_mm/2, height=height_mm) centered at the origin,
    # standing along local Z by default (same shape/param convention as round_post, confirmed by
    # reading this file's _build directly) -- its two flat end faces are real, useful mount points
    # (e.g. stacking a wheel_blank on a hub/shaft end).
