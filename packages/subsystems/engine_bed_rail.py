"""Engine Bed Rail -- Twin-rail engine bed (glow/gas engines) -- one rail; place two instances

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
5) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Engine Bed Rail
Twin-rail engine bed (glow/gas engines) -- one rail; place two instances -- a solid rectangular bar (FDM/FFF or CNC).
...
ENGINE_BED_RAIL = register_subsystem(Subsystem(
    name="engine_bed_rail",
    description="Twin-rail engine bed (glow/gas engines) -- one rail; place two instances -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=120.0, min=30.0, max=400.0, unit='mm'),
        ParamSpec("width_mm", value=8.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=4.0, max=60.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # 2026-07-28 — plain Box along local X, same shape family as flat_bar/longeron
))
