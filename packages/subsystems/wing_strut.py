"""Wing Strut -- External brace strut (strut-braced wing)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
2) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Wing Strut
External brace strut (strut-braced wing) -- a solid rectangular bar (FDM/FFF or CNC).
...
WING_STRUT = register_subsystem(Subsystem(
    name="wing_strut",
    description="External brace strut (strut-braced wing) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=350.0, min=80.0, max=1200.0, unit='mm'),
        ParamSpec("width_mm", value=10.0, min=3.0, max=50.0, unit='mm'),
        ParamSpec("height_mm", value=6.0, min=2.0, max=30.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # 2026-07-28 — same centered-Box-along-X bar shape as flat_bar/longeron
))
