"""Stringer -- Thin structural rail, lighter than a longeron, many per frame

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
1) -- a solid rectangular bar, the SAME shape family `longeron.py`/`flat_bar.py` already
...
_FRAGMENT = """\
## Subsystem: Stringer
Thin structural rail, lighter than a longeron, many per frame -- a solid rectangular bar (FDM/FFF or CNC).
...
STRINGER = register_subsystem(Subsystem(
    name="stringer",
    description="Thin structural rail, lighter than a longeron, many per frame -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=400.0, min=50.0, max=1500.0, unit='mm'),
        ParamSpec("width_mm", value=8.0, min=3.0, max=40.0, unit='mm'),
        ParamSpec("height_mm", value=4.0, min=0.8, max=20.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # 2026-07-28 -- same Box(length,width,height)-along-X shape family as flat_bar/longeron
))
