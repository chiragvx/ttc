"""Endcap Round -- Cap that fits over a round tube OD (modeled as a through-bore ring rather than a blind pocket cap)
...
_FRAGMENT = """\
## Subsystem: Endcap Round
Cap that fits over a round tube OD (modeled as a through-bore ring rather than a blind pocket cap) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
ENDCAP_ROUND = register_subsystem(Subsystem(
    name="endcap_round",
    description="Cap that fits over a round tube OD -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=20.0, min=5.0, max=75.0, unit='mm'),
        ParamSpec("height_mm", value=15.0, min=3.0, max=40.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
