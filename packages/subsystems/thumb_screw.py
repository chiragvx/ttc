"""Thumb Screw -- Knurled cylindrical head + shank (knurl/thread not modeled)
...
_FRAGMENT = """\
## Subsystem: Thumb Screw
Knurled cylindrical head + shank (knurl/thread not modeled) -- a two-diameter stepped/shouldered cylinder, optional through-bore.
...
def _head_face(p):
    ...
    This is the underside of the (typically larger) knurled head -- the face that seats flush against
    whatever the thumb screw bears on -- so its outward normal points -Z (away from the part)."""
...
THUMB_SCREW = register_subsystem(Subsystem(
    name="thumb_screw",
    description="Knurled cylindrical head + shank -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia1_mm", value=12.0, min=4.0, max=35.0, unit='mm'),
        ParamSpec("dia2_mm", value=4.0, min=1.5, max=18.0, unit='mm'),
        ParamSpec("len1_mm", value=6.0, min=2.0, max=20.0, unit='mm'),
    ...
    # knurled head, z=0..len1_mm) with a narrower section2 (the shank) stacked on top
    ...
    # part's own real z-offsets instead, same pattern as `button_head_bolt_blank`/`T_nut`. `head_face`
