"""Flat Shim -- Very thin flat plate
...
_FRAGMENT = """\
## Subsystem: Flat Shim
Very thin flat plate -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
FLAT_SHIM = register_subsystem(Subsystem(
    name="flat_shim",
    description="Very thin flat plate -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("width_mm", value=30.0, min=8.0, max=120.0, unit='mm'),
        ParamSpec("depth_mm", value=20.0, min=6.0, max=90.0, unit='mm'),
        ParamSpec("thickness_mm", value=0.8, min=0.3, max=4.0, unit='mm'),
        ParamSpec("hole_dia_mm", value=3.0, min=1.0, max=8.0, unit='mm'),
        ParamSpec("n_holes", value=0, min=0, max=4, unit='count'),
