"""Cubesat Rail -- Corner rail: square section, mounting-hole pitch (more accurate than reusing square_tube)

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
12) -- an extruded hollow square section, the SAME shape family `square_tube.py` already
...
_FRAGMENT = """\
## Subsystem: Cubesat Rail
Corner rail: square section, mounting-hole pitch (more accurate than reusing square_tube) -- an extruded hollow square section (FDM/FFF or extruded stock).
...
CUBESAT_RAIL = register_subsystem(Subsystem(
    name="cubesat_rail",
    description="Corner rail: square section, mounting-hole pitch (more accurate than reusing square_tube) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("length_mm", value=100.0, min=20.0, max=350.0, unit='mm'),
        ParamSpec("outer_side_mm", value=8.5, min=3.0, max=40.0, unit='mm'),
        ParamSpec("wall_thickness_mm", value=1.6, min=0.8, max=8.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    interfaces=bar_end_interfaces("length_mm"),  # Box(length,outer_side,outer_side) centered at
    # origin, length along local X — the exact bar_end_interfaces convention (confirmed against _build).
))
