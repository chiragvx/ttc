"""Button Head Bolt Blank -- Dome head + shank (dome/thread not modeled)
...
_FRAGMENT = """\
## Subsystem: Button Head Bolt Blank
Dome head + shank (dome/thread not modeled) -- a two-diameter stepped/shouldered cylinder, optional through-bore.
...
BUTTON_HEAD_BOLT_BLANK = register_subsystem(Subsystem(
    name="button_head_bolt_blank",
    description="Dome head + shank -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("dia1_mm", value=8.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("dia2_mm", value=4.0, min=1.5, max=18.0, unit='mm'),
        ParamSpec("len1_mm", value=3.0, min=1.0, max=10.0, unit='mm'),
    ...
    # z-offsets instead, same pattern as `T_nut`'s flange/shaft. `head_face` is the head's real
    # bearing face; `tip_face` is the shank's free end.
    interfaces=[
