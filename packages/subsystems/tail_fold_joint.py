"""Tail Fold Joint -- Static fold joint for a folding tail (transport/storage) -- one leaf
...
_FRAGMENT = """\
## Subsystem: Tail Fold Joint
Static fold joint for a folding tail (transport/storage) -- one leaf -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
TAIL_FOLD_JOINT = register_subsystem(Subsystem(
    name="tail_fold_joint",
    description="Static fold joint for a folding tail (transport/storage) -- one leaf -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
