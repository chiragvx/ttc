"""Pillow Block Housing -- Pillow-block bearing shell (bearing not modelled) (modeled as a flanged housing; the bearing itself is not modeled)

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
8) -- a cylindrical shaft/boss with a wider mounting flange at its base, a concentric bore,
...
_FRAGMENT = """\
## Subsystem: Pillow Block Housing
Pillow-block bearing shell (bearing not modelled) (modeled as a flanged housing; the bearing itself is not modeled) -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
def _flange_face(p):
    """The flange's OUTER (bottom) face, z=0 -- where this housing bolts flat against a mounting
...
PILLOW_BLOCK_HOUSING = register_subsystem(Subsystem(
    name="pillow_block_housing",
    description="Pillow-block bearing shell (bearing not modelled) -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
