"""Castor Blank -- Wheel + fork mount + swivel post (represents the wheel hub only; fork mount/swivel post not modeled)
...
_FRAGMENT = """\
## Subsystem: Castor Blank
Wheel + fork mount + swivel post (represents the wheel hub only; fork mount/swivel post not modeled) -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
CASTOR_BLANK = register_subsystem(Subsystem(
    name="castor_blank",
    description="Wheel + fork mount + swivel post -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=25.0, min=8.0, max=80.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=5.0, min=1.5, max=40.0, unit='mm'),
