"""Cotter Pin Slot -- A plate with a slot sized for a cotter pin (slot approximated as a round hole)
...
_FRAGMENT = """\
## Subsystem: Cotter Pin Slot
A plate with a slot sized for a cotter pin (slot approximated as a round hole) -- a flat plate with a row of bolt holes (FDM/FFF or CNC).
...
COTTER_PIN_SLOT = register_subsystem(Subsystem(
    name="cotter_pin_slot",
    description="A plate with a slot sized for a cotter pin -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    ...
    params=[
        ...
        ParamSpec("hole_dia_mm", value=2.5, min=1.0, max=6.0, unit='mm'),
