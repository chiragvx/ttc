"""Payload Bay Ring -- Ring/frame mounting a cylindrical payload inside the bus

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
6) -- a flat ring frame + evenly-spaced bolt-hole pattern, the SAME shape family
...
_FRAGMENT = """\
## Subsystem: Payload Bay Ring
Ring/frame mounting a cylindrical payload inside the bus -- a flat ring frame perpendicular to the fuselage axis (FDM/FFF or CNC).
...
PAYLOAD_BAY_RING = register_subsystem(Subsystem(
    name="payload_bay_ring",
    description="Ring/frame mounting a cylindrical payload inside the bus -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=100.0, min=30.0, max=350.0, unit='mm'),
        ParamSpec("flange_width_mm", value=9.0, min=3.0, max=30.0, unit='mm'),
