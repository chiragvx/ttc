"""Control Rod Coupler -- Coupler joining two control-rod sections

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
8) -- a cylinder with a concentric through-bore, the SAME shape family `standoff.py`
...
_FRAGMENT = """\
## Subsystem: Control Rod Coupler
Coupler joining two control-rod sections -- a cylindrical body with a concentric through-bore (FDM/FFF or turned).
...
CONTROL_ROD_COUPLER = register_subsystem(Subsystem(
    name="control_rod_coupler",
    description="Coupler joining two control-rod sections -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("outer_dia_mm", value=6.0, min=3.0, max=25.0, unit='mm'),
        ParamSpec("inner_dia_mm", value=2.5, min=1.0, max=15.0, unit='mm'),
