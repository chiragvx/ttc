"""Recovery Harness Anchor -- Anchor point for a recovery harness/parachute line

Structural/mounting geometry only (`build-plan/reference/UAV_SUBSYSTEM_PROPOSALS.md` category
10) -- two perpendicular flanges sharing a corner, the SAME shape family `lbracket.py`
...
_FRAGMENT = """\
## Subsystem: Recovery Harness Anchor
Anchor point for a recovery harness/parachute line -- an angle bracket, two perpendicular flanges meeting at a corner (FDM/FFF or machined).
...
RECOVERY_HARNESS_ANCHOR = register_subsystem(Subsystem(
    name="recovery_harness_anchor",
    description="Anchor point for a recovery harness/parachute line -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("leg_a_mm", value=15.0, min=6.0, max=50.0, unit='mm'),
        ParamSpec("leg_b_mm", value=15.0, min=6.0, max=50.0, unit='mm'),
        ParamSpec("width_mm", value=10.0, min=4.0, max=40.0, unit='mm'),
