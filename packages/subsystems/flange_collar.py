"""Flange Collar -- Collar with an outboard mounting flange

Structural/mounting geometry only (`build-plan/reference/SUBSYSTEM_PROPOSALS.md` category
7) -- a cylindrical shaft/boss with a wider mounting flange at its base, a concentric bore,
...
_FRAGMENT = """\
## Subsystem: Flange Collar
Collar with an outboard mounting flange -- a shaft/boss on a wider mounting flange, concentric bore, flange bolt-hole pattern.
...
def _flange_face(p) -> "Frame":
    """The flange's OUTER (bottom) face, z=0 -- where this collar bolts flat against a mounting
    surface (the `n_bolt_holes` pattern passes through the flange right at this face). Confirmed
    against `_build`: `flange = bd.Pos(0, 0, flange_thickness_mm/2) * bd.Cylinder(...)`, i.e. the
    flange spans z in [0, flange_thickness_mm] -- NOT centered at the origin the way
...
FLANGE_COLLAR = register_subsystem(Subsystem(
    name="flange_collar",
    description="Collar with an outboard mounting flange -- structural/mounting geometry (FDM/FFF or CNC)",
    fragment=_FRAGMENT,
    disciplines=("structures", "manufacturing", "thermal"),
    params=[
        ParamSpec("shaft_dia_mm", value=14.0, min=5.0, max=45.0, unit='mm'),
        ParamSpec("shaft_len_mm", value=15.0, min=4.0, max=50.0, unit='mm'),
        ParamSpec("flange_dia_mm", value=30.0, min=10.0, max=90.0, unit='mm'),
        ParamSpec("flange_thickness_mm", value=3.0, min=0.8, max=12.0, unit='mm'),
        ParamSpec("bore_dia_mm", value=6.0, min=1.5, max=30.0, unit='mm'),
        ParamSpec("n_bolt_holes", value=3, min=0, max=8, unit='count'),
        ParamSpec("bolt_hole_dia_mm", value=3.4, min=1.5, max=8.0, unit='mm'),
    ],
    build=_build,
    volume=_volume,
    invariants=_check,
    ...
    # `shaft_tip` is the boss's free end. See `_flange_face`/`_shaft_tip` above for the geometry math.
    interfaces=[
